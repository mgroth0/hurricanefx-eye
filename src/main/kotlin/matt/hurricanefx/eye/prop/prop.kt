package matt.hurricanefx.eye.prop
...
fun <T> Any.getProperty(prop: KMutableProperty1<*, T>): ObjectProperty<T> {
  ...
  val field =
	  requireNotNull(javaClass.findFieldByName("${prop.name}\$delegate")) { "No delegate field with name '${prop.name}' found" }
...
/**
 * Convert the given key in this map to a matt.hurricanefx.Property using the given propertyGenerator function.
 *
 * The generator is passed the initial value corresponding to the given key.
 *
 * Changes to the generated matt.hurricanefx.Property will automatically be written back into the map.
 */
@Suppress("UNCHECKED_CAST")
